...
void ordenaBoletosValorPago(struct Boleto *, int);
...
int main(){
    ...
    for(i=0; i<n; i++){
        scanf("%s %s %s %lf %d %d %lf", banco, cedente, sacado, &valor, &diaVen, &diaPag, &multa);
        vBol[i] = criaBoleto(banco, cedente, sacado, valor, diaVen, diaPag, multa);
    }
    ordenaBoletosValorPago(vBol, n);
...
void ordenaBoletosValorPago(struct Boleto *v, int n){
    int i, j;
    struct Boleto aux;
